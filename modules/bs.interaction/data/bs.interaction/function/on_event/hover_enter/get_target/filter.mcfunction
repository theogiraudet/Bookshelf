...
execute store result score #s bs.ctx if predicate {condition:entity_properties,entity:this,predicate:{type_specific:{type:player,looking_at:{type:interaction,nbt:"{Tags:[bs.interaction.mask]}",distance:{absolute:{max:24}}}}}}
