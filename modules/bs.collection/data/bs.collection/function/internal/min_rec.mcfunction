...
# Update minimum if current is smaller - copy both score and original value
...
execute if data storage bs:data collection.stack[0].value[0] run function bs.collection:internal/min_rec with storage bs:data collection.stack[0]
