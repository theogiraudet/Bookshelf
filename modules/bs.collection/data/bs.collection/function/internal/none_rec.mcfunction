...
# If the predicate succeeded, set result to false and short-circuit
...
execute if score #s bs.ctx matches 0 if data storage bs:data collection.stack[0].value[0] run return run function bs.collection:internal/none_rec

# If the predicate never succeeded and there are no more elements, return true
