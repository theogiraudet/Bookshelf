...
execute store success score #s bs.ctx run function bs.collection:filter/call with storage bs:data collection.stack[0]

# If the predicate succeeded, keep the element in the result
...
execute if data storage bs:data collection.stack[0].value[0] run function bs.collection:filter/filter_rec
