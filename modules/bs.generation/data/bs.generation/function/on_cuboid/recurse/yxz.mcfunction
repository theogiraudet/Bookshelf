...
$execute if score $generation.i bs.lambda < #generation.h bs.data positioned ~ ~$(y) ~ run return run function bs.generation:on_cuboid/recurse/yxz with storage bs:data generation[-1]
scoreboard players set $generation.i bs.lambda 0
scoreboard players add $generation.j bs.lambda 1
$execute if score $generation.j bs.lambda < #generation.w bs.data positioned ~$(x) $(sy) ~ run return run function bs.generation:on_cuboid/recurse/yxz with storage bs:data generation[-1]
scoreboard players set $generation.j bs.lambda 0
scoreboard players add $generation.k bs.lambda 1
$execute if score $generation.k bs.lambda < #generation.d bs.data positioned $(sx) $(sy) ~$(z) run return run function bs.generation:on_cuboid/recurse/yxz with storage bs:data generation[-1]
