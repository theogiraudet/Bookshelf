...
data modify storage bs:data generation[-1].psr set value "bs.generation:callbacks/fractal_noise_2d/psr"
data modify storage bs:data generation[-1] merge value { size: 32, octaves: 2, persistence: .5, lacunarity: 2 }

$data modify storage bs:data generation[-1] merge value $(with)
$data modify storage bs:data generation[-1].cb set value '$(run)'

execute unless data storage bs:data generation[-1].seed store result storage bs:data generation[-1].seed int 1 run random value 1.. bs.generation:fractal_noise_2d
data modify storage bs:data generation[-1].run set value 'function bs.generation:callbacks/fractal_noise_2d/run with storage bs:data generation[-1]'
...
function bs.generation:callbacks/fractal_noise_2d/run with storage bs:data generation[-1]
