...
data modify storage bs:ctx _ set from storage bs:in spline.sample_bspline
data modify storage bs:out spline.sample_bspline set value []
...
execute if score #n bs.ctx = #m bs.ctx run return run function bs.spline:sample/sample_3d {type:"bspline"}
execute store result score #n bs.ctx if data storage bs:ctx _.points[][1]
execute if score #n bs.ctx = #m bs.ctx run return run function bs.spline:sample/sample_2d {type:"bspline"}
execute store result score #n bs.ctx if data storage bs:ctx _.points[][0]
execute if score #n bs.ctx = #m bs.ctx run return run function bs.spline:sample/sample_1d {type:"bspline"}
