...
data modify storage bs.animation: _ set value {k:3,r:2,i:0,t:0f}
...
execute store result storage bs.animation: _.n int 1 run return run compute default integer {type:max,inputs:[{type:min,inputs:[{type:sub,left:{type:floor_div,left:{type:score,target:{type:fixed,name:"#n"},score:"bs.ctx"},right:2},right:1},{type:storage,storage:"bs.animation:",path:"_.n",fallback:2147483647}]},0]}
