# If this is a global function transition, we run the function and according to the result, we switch to the target state
execute if data storage bs:data fsm.listened_transitions[0].global \
        store success score #s bs.ctx \
        run function bs.fsm:run/run_command_global with storage bs:data fsm.listened_transitions[0].condition
...
# If this is a local function transition, we run the function and according to the result, we switch to the target state
...
execute unless data storage bs:data fsm.listened_transitions[0].global \
        store success score #s bs.ctx \
        run function bs.fsm:run/run_command_local with storage bs:data fsm.listened_transitions[0]
execute if score #s bs.ctx matches 1 run function bs.fsm:run/switch_state with storage bs:ctx _.tmp
execute if score #s bs.ctx matches 1 run return 1
